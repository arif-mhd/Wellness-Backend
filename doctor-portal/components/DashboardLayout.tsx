...
function GlobalSearch() {
  ...
  return (
    <div ref={wrapperRef} className="hidden sm:block flex-1 max-w-[605px] mx-2 sm:mx-8 relative">
      {/* Input */}
  ...
  return (
    <div className="flex h-screen bg-[#F7F9FC] overflow-x-hidden relative font-sans">
      ...
      <div className="flex-1 flex flex-col min-w-0 z-10 h-full">
        <header className={`h-[96px] flex items-center justify-between shrink-0 select-none transition-all duration-300 ${isVideoCall ? "px-3 sm:px-6 lg:px-[40px]" : sidebarOpen ? "px-3 sm:px-6 xl:px-[24px]" : "px-3 sm:px-6 lg:px-[40px]"
          }`}>
          {/* Logo */}
          ...
          <div className="flex items-center gap-2.5 sm:gap-3">
            ...
            <button
              ...
              className="relative h-[44px] w-[44px] sm:h-[48px] sm:w-auto px-0 sm:px-5 bg-gradient-to-b from-[#FF6B6B] to-[#E84949] hover:from-[#FF7A7A] hover:to-[#D63D3D] text-white rounded-xl text-[13px] font-medium flex items-center justify-center sm:gap-2.5 shadow-[0_4px_10px_rgba(232,73,73,0.2)] transition-all active:scale-95 select-none shrink-0"
            >
              <svg width="19" height="17" viewBox="0 0 19 17" fill="none" className="shrink-0">
              ...
              <span className="hidden sm:block whitespace-nowrap">SOS Records</span>
            </button>

            {/* Waiting Room */}
            <button
              onClick={() => router.push("/appointments/waitingroom")}
              className="relative h-[44px] w-[44px] sm:h-[48px] sm:w-auto px-0 sm:px-5 bg-gradient-to-b from-[var(--brand-secondary,#8AA0FF)] to-[var(--brand-primary,#5476FC)] text-white rounded-xl text-[13px] font-bold flex items-center justify-center sm:gap-2.5 shadow-[0_6px_20px_rgba(84,118,252,0.25)] hover:shadow-[0_8px_24px_rgba(84,118,252,0.35)] transition-all select-none shrink-0"
            >
              ...
              <span className="hidden sm:block whitespace-nowrap">Waiting Room{waitingCount > 0 ? ` (${waitingCount})` : ""}</span>
              {/* Mobile Badge for Waiting Count */}
              <div className="sm:hidden">
                {waitingCount > 0 && (
                  <span className="absolute -top-1.5 -right-1.5 w-5 h-5 bg-white text-[#5476FC] text-[10px] font-bold rounded-full flex items-center justify-center shadow-md border border-[#5476FC]/10">
                    {waitingCount}
                  </span>
                )}
              </div>
            ...
            <div className="relative">
              <button
                ...
                className="w-11 h-11 sm:w-12 sm:h-12 bg-white hover:bg-gray-50 rounded-full flex items-center justify-center text-[#3D4B5A] border border-[#EBEEF5] transition-all relative"
              >
                ...
                {unreadCount > 0 && (
                  <span className="absolute top-1.5 right-1.5 sm:top-2 sm:right-2 w-4 h-4 bg-[#E84949] text-white text-[9px] font-bold rounded-full flex items-center justify-center border border-white">
                    {unreadCount}
                  </span>
                )}
              </button>
                      ...
                      <div className="flex items-center gap-6">
                        {(["Unread", "All"] as const).map((tab) => (
                          <button
                            ...
                            className={`pb-3 text-[13px] font-bold transition-colors relative ${notifTab === tab ? "text-slate-800" : "text-slate-400 hover:text-slate-600"
                              }`}
                          >
                            {tab}
                            {notifTab === tab && (
            ...
            <div className="relative">
              <button
                ...
                className="w-11 h-11 sm:w-12 sm:h-12 bg-white hover:bg-gray-50 rounded-full flex items-center justify-center text-[#3D4B5A] border border-[#EBEEF5] transition-all"
              >
                <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
