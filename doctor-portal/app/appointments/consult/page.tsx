      ...
      <div className="flex flex-col xl:flex-row flex-1 overflow-y-auto xl:overflow-hidden">
        ...
        <div className="flex flex-col w-full xl:w-[560px] xl:flex-shrink-0">
          ...
          >
            ...
            {/* Fullscreen floating chat button */}
            <button
            ...
            >
              <div className="flex items-center justify-between px-4 py-3 border-b border-white/10 flex-shrink-0">
                <div className="flex items-center gap-2">
                  <svg width="13" height="13" fill="none" viewBox="0 0 24 24" stroke="white" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" /></svg>
                ...
                {messages.length === 0 ? (
                  ...
                  <div key={m.id} className={`flex items-start gap-2 ${m.sender === "you" ? "flex-row-reverse" : ""}`}>
                    ...
                    <div className={`max-w-[80%] flex flex-col gap-0.5 ${m.sender === "you" ? "items-end" : "items-start"}`}>
                      ...
                      <div className={`px-2.5 py-1.5 rounded-xl text-[11px] leading-relaxed ${m.sender === "you" ? "bg-[#5476fc] text-white rounded-tr-none" : "bg-white/10 text-white/90 rounded-tl-none"}`}>
                        {m.text}
                      </div>
              ...
              <div className="p-3 border-t border-white/10 flex gap-2 items-center flex-shrink-0">
                ...
                <button onClick={sendChat} disabled={!chatInput.trim()}
                  ...
                  <svg width="10" height="10" fill="none" viewBox="0 0 24 24"><path d="M22 2L11 13M22 2L15 22l-4-9-9-4 20-7z" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg>
                </button>
              </div>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-hidden flex flex-col border-l border-gray-100">
          ...
          {/* Tab bar */}
          <div className="flex items-center border-b border-gray-100 flex-shrink-0 px-2">
            {([
              { key: "chat" as const, label: "Chat" },
              { key: "emr" as const, label: "EMR" },
              { key: "specialist" as const, label: "Add Specialist" },
            ]).map(({ key, label }) => (
              <button
                key={key}
                onClick={() => {
                ...
                className={`relative px-4 py-3 text-xs font-semibold transition-colors ${rightTab === key ? "text-[#5476fc]" : "text-gray-400 hover:text-gray-600"
                  }`}
              >
                {label}
              ...
              <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3" onClick={() => setUnread(0)}>
                {messages.length === 0 ? (
                  <div className="flex flex-col items-center justify-center h-full text-center">
                    <svg width="32" height="32" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5" className="text-gray-200 mb-3">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                    </svg>
                    <p className="text-gray-300 text-[12px] font-medium">No messages yet</p>
                    <p className="text-gray-200 text-[11px] mt-1">Start a conversation with {patientName}</p>
                  </div>
                ) : messages.map(m => (
                  <div key={m.id} className={`flex items-start gap-2 ${m.sender === "you" ? "flex-row-reverse" : ""}`}>
              ...
              <div className="px-4 py-3 border-b border-gray-100 flex-shrink-0">
                <div className="relative">
                  ...
                  <input value={specialistSearch} onChange={e => setSpecialistSearch(e.target.value)}
                    ...
                    className="w-full bg-gray-50 rounded-xl pl-8 pr-3 py-2.5 text-xs text-gray-700 placeholder-gray-300 outline-none border border-transparent focus:border-[#5476fc]/30 transition-colors" />
                </div>
              </div>
