...
async function uploadFiles(
  ...
  const res = await apiFetch("/api/admin/clinics/upload", {
    method: "POST",
    body: form,
  });
...
const STEPS = [
  { key: 1, label: "Owner's Personal Information" },
  { key: 2, label: "Insurances" },
  { key: 3, label: "Clinic / Company Information" },
  { key: 4, label: "Set Availability" },
  { key: 5, label: "Set Password" },
];
...
export default function AddClinicPage() {
  ...
  const handleFinalSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error("Add clinic submit error:", err);
      ...
      <div className="max-w-[1440px] mx-auto space-y-7 pb-12 font-sans px-1 animate-in fade-in duration-300">
        ...
        {submitting && (
          <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-white/80 backdrop-blur-sm gap-4">
            ...
            <p className="text-gray-600 text-sm font-medium">Creating clinic account…</p>
          </div>
        )}
