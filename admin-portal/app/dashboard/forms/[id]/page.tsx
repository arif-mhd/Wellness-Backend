...
function makeField(type: FieldType): FormField {
  ...
  const placeholders: Partial<Record<FieldType, string>> = {
    text: 'Enter name', email: 'Enter email address', phone: 'Enter phone number',
    address: 'Enter address', number: 'Enter a number', textarea: 'Enter detailed response',
  };
  return {
    id: makeId(),
    type,
    label: type === 'heading' ? 'Section Heading' : `${elem.label} Question`,
    placeholder: placeholders[type] || `Enter ${elem.label.toLowerCase()}`,
    required: false,
    subLabel: '',
...
function FieldPreview({ field, onUpdate }: { field: FormField; onUpdate: (u: Partial<FormField>) => void }) {
  ...
  switch (field.type) {
    ...
    case 'email': return <input type="email" placeholder={field.placeholder} className={base} />;
    case 'textarea': return <textarea placeholder={field.placeholder} rows={3} className={`${base} resize-none`} />;
    case 'dropdown':
      return (
        <div className="relative">
          <select className={`${base} appearance-none pr-10`}>
            <option value="">Please select options</option>
            {field.options.map(o => <option key={o.id}>{o.value}</option>)}
          </select>
        ...
        <div className="bg-[#f8fafd] rounded-xl p-4 border border-slate-100 space-y-3">
          {field.options.map((o, i) => (
            <label key={o.id} className="flex items-center gap-3 cursor-pointer">
              ...
              <span className="text-[13px] font-medium text-slate-700">{o.value || `Option ${i+1}`}</span>
            </label>
          ))}
        </div>
      );
...
const INITIAL: FormField[] = [
  { id: 'f1', type: 'text', label: 'Patient Full Name', placeholder: 'Enter full name', required: true, subLabel: 'As it appears on your ID', options: [] },
  { id: 'f2', type: 'email', label: 'Email Address', placeholder: 'Enter email', required: true, subLabel: '', options: [] },
  { id: 'f3', type: 'phone', label: 'Phone Number', placeholder: '+1 (555) 000-0000', required: false, subLabel: '', options: [] },
  { id: 'f4', type: 'dropdown', label: 'Department', placeholder: '', required: true, subLabel: '', options: [{ id: 'o1', value: 'Cardiology' }, { id: 'o2', value: 'Neurology' }, { id: 'o3', value: 'Pediatrics' }] },
  { id: 'f5', type: 'date', label: 'Date of Birth', placeholder: '', required: true, subLabel: '', options: [] },
  { id: 'f6', type: 'radio', label: 'Have you visited before?', placeholder: '', required: false, subLabel: '', options: [{ id: 'r1', value: 'Yes' }, { id: 'r2', value: 'No' }] },
  { id: 'f7', type: 'textarea', label: 'Symptoms / Notes', placeholder: 'Describe your symptoms...', required: false, subLabel: '', options: [] },
];

export default function EditFormPage({ params }: { params: Promise<{ id: string }> }) {
  const router = useRouter();
  const { id } = use(params);
  const [formTitle, setFormTitle] = useState('Patient Prescription Form');
  ...
  const updateActive = (updates: Partial<FormField>) => { if (activeId) updateField(activeId, updates); };
  ...
  const removeField = (fid: string, e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  const duplicateField = () => {
    ...
    setFields(fs => { const i = fs.findIndex(f => f.id === activeId); const n = [...fs]; n.splice(i + 1, 0, copy); return n; });
    setActiveId(copy.id);
  };

  const addOption = () => { if (!active) return; updateActive({ options: [...active.options, { id: makeId(), value: `Option ${active.options.length + 1}` }] }); };
  const removeOption = (oid: string) => { if (!active) return; updateActive({ options: active.options.filter(o => o.id !== oid) }); };
  const updateOption = (oid: string, value: string) => { if (!active) return; updateActive({ options: active.options.map(o => o.id === oid ? { ...o, value } : o) }); };

  const isFullWidth = (type: FieldType) => ['radio', 'checkbox', 'textarea', 'photo', 'signature', 'appointment', 'address'].includes(type);
              ...
              {fields.map(field => {
                ...
                return (
                  ...
                  >
                    ...
                    <FieldPreview field={field} onUpdate={u => updateField(field.id, u)} />
                    {isActive && (
