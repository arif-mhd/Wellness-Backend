import { CurrencyConfig } from "@/components/CountryConfigContext";
...
// isn't trying to model). Replaces every hardcoded "AED" literal with a
// currency read from the org's country config, so an India-scoped org shows
// INR instead of AED with no per-screen special-casing.
