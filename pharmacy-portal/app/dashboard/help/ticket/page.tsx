...
interface TicketComment {
  id: string;
  authorRole: "admin" | "pharmacy" | "patient";
  message: string;
  createdAt: string;
}
...
function TicketContent() {
  ...
  const updates: Array<{ id: string; sender: "support" | "pharmacy"; message: string; dateTime: string; sortKey: string }> = [];
  ...
  for (const c of ticket.comments ?? []) {
    updates.push({
      id: c.id,
      sender: c.authorRole === "admin" ? "support" : "pharmacy",
      message: c.message,
            ...
            <div className="bg-[#F9FAFC] border border-[#EBEEF5] rounded-[24px] p-6 flex flex-col gap-6 shadow-sm">
              ...
              {updates.length === 0 ? (
                ...
                <div className="flex flex-col gap-6 w-full">
                  {updates.map((update) => {
                    const isPharmacy = update.sender === "pharmacy";
                    return (
                      <div key={update.id} className={`flex gap-4 items-start w-full ${isPharmacy ? "flex-row-reverse" : ""}`}>
                        <div className={`w-9 h-9 rounded-full border flex items-center justify-center shrink-0 shadow-sm ${
                          isPharmacy ? "border-[#E0E7FF] bg-[#E0E7FF] text-[#182A6F]" : "border-[#EBF2FC] bg-[#EBF2FC] text-[#5476FC]"
                        }`}>
                        ...
                        <div className={`flex flex-col gap-1 flex-1 ${isPharmacy ? "items-end" : "items-start"}`}>
                          <div className={`bg-white border border-[#EBEEF5] rounded-[16px] p-4 shadow-[0_2px_8px_rgba(0,0,0,0.02)] max-w-[85%]`}>
                            <p className="text-[10px] font-bold mb-1" style={{ fontFamily: "Outfit, sans-serif", color: isPharmacy ? "#182A6F" : "#5476FC" }}>
                              {isPharmacy ? "You" : "Wellness Central Support"}
                            </p>
                            <p
